...
# Модуль 1 + Модуль 2 + Модуль 3 (КНОПКА ВСЕГДА ВИДНА)
# ============================================
...
# Пытаемся импортировать библиотеки для QR-кода
try:
    import qrcode
    ...
    QR_AVAILABLE = False
    print("⚠️  Для работы QR-кода установите: pip install qrcode[pil] Pillow")
...
# ============================================
# РАБОТА С БАЗОЙ ДАННЫХ (МОДУЛЬ 2)
# ============================================
...
class Database:
        ...
        with self.get_connection() as conn:
            ...
            cursor.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS Users (
                    ...
                    Role TEXT NOT NULL CHECK(Role IN ('Менеджер', 'Автомеханик', 'Оператор', 'Заказчик', 'Менеджер по качеству')),
            ...
            if need_data:
                ...
                print("✅ Добавлен Менеджер по качеству: quality1 / pass1")
            else:
    ...
    def insert_test_data(self, conn):
        ...
        # Пользователи (ДОБАВЛЕН МЕНЕДЖЕР ПО КАЧЕСТВУ)
        users_data = [
            ...
            (11, 'Соколова Елена Викторовна', '89997654321', 'quality1', 'pass1', 'Менеджер по качеству')
        ]
    ...
    def get_requests(self, user_id: Optional[int] = None, role: Optional[str] = None) -> List[Request]:
        ...
        if result:
            for row in result:
                # Преобразуем даты
                if 'StartDate' in row and isinstance(row['StartDate'], str):
                    pass
...
# ============================================
# КЛАСС ДЛЯ ГЕНЕРАЦИИ QR-КОДА (МОДУЛЬ 3)
# ============================================

class QRCodeGenerator:
    """Окно с QR-кодом для опроса клиентов"""
    
    def __init__(self, parent):
        self.parent = parent
        self.survey_url = "https://docs.google.com/forms/d/e/1FAIpQLSdhZcExx6LSIXxk0ub55mSu-WIh23WYdGG9HY5EZhLDo7P8eA/viewform"
        
        # Проверяем доступность библиотек
        if not QR_AVAILABLE:
            messagebox.showerror(
                "Ошибка",
                "Библиотеки для QR-кода не установлены.\n\n"
                "Установите команду:\n"
    ...
    def create_window(self):
        """Создаёт окно с QR-кодом"""
        try:
            # Создаём новое окно
            self.window = tk.Toplevel(self.parent)
            self.window.title("📱 Оцените качество обслуживания")
            ...
            # Заголовок
            ttk.Label(self.window, text="Оцените качество работы", 
                     font=('Arial', 16, 'bold')).pack(pady=15)
            
            ttk.Label(self.window, text="Отсканируйте QR-код и пройдите опрос",
            ...
            # Добавляем ссылку
            qr.add_data(self.survey_url)
            qr.make(fit=True)
            
            # Создаём картинку
            qr_image = qr.make_image(fill_color="black", back_color="white")
            
            # Уменьшаем размер для экрана
            ...
            # Показываем QR-код
            ttk.Label(self.window, image=self.qr_photo).pack(pady=10)
            ...
            ttk.Button(btn_frame, text="💾 Сохранить", 
                      command=save_image, width=15).pack(side='left', padx=5)
            
            # Кнопка "Открыть ссылку"
            def open_link():
                webbrowser.open(self.survey_url)
            
            ttk.Button(btn_frame, text="🔗 Открыть ссылку", 
                      command=open_link, width=15).pack(side='left', padx=5)
            
            # Кнопка "Закрыть"
            ttk.Button(btn_frame, text="✖ Закрыть", 
                      command=self.window.destroy, width=15).pack(side='left', padx=5)
            
            # Пояснение
            info_frame = ttk.LabelFrame(self.window, text="Информация", padding=10)
            info_frame.pack(fill='x', padx=20, pady=10)
            
            ttk.Label(info_frame, 
                     text="Клиенты могут отсканировать этот код\n"
                          "с помощью камеры телефона и оценить\n"
...
# ============================================
# ГРАФИЧЕСКИЙ ИНТЕРФЕЙС (МОДУЛЬ 1)
# ============================================
...
class AutoServiceApp:
    ...
    def show_login(self):
        ...
        ⭐ Менеджер по качеству: quality1 / pass1  (НОВОЕ!)
        """
    ...
    def show_main_menu(self):
        ...
        # ДЛЯ МЕНЕДЖЕРА ПО КАЧЕСТВУ (МОДУЛЬ 3)
        if self.current_user.Role == 'Менеджер по качеству':
            buttons.append(("⏱ ПРОДЛИТЬ СРОК", self.extend_deadline))
            buttons.append(("👥 ПРИВЛЕЧЬ МЕХАНИКОВ", self.assign_additional_mechanic))
        
        # ДЛЯ ВСЕХ ПОЛЬЗОВАТЕЛЕЙ - QR-код ВСЕГДА ДОБАВЛЯЕМ (УБРАЛ ПРОВЕРКУ QR_AVAILABLE)
        buttons.append(("📱 QR-КОД ОПРОСА", self.show_qr_code))
    ...
    # ========== МЕТОДЫ ДЛЯ МЕНЕДЖЕРА ПО КАЧЕСТВУ (МОДУЛЬ 3) ==========
    ...
    def extend_deadline(self):
        ...
        ttk.Label(dialog, text="⏱ ПРОДЛЕНИЕ СРОКА ЗАЯВКИ", 
                 font=('Arial', 14, 'bold')).pack(pady=10)
        
        # Получаем активные заявки (в работе)
        all_requests = self.db.get_requests()
        active_requests = []
        for r in all_requests:
            if r.RequestStatus in ['В процессе ремонта', 'Ожидание автозапчастей']:
                active_requests.append(r)
        
        if not active_requests:
            ttk.Label(dialog, text="Нет активных заявок для продления").pack(pady=20)
            ttk.Button(dialog, text="Закрыть", command=dialog.destroy).pack()
            return
        
        # Выбор заявки
        ttk.Label(dialog, text="Выберите заявку:").pack(pady=5)
        
        # Создаём список для выбора
        request_list = []
        for r in active_requests:
            client = self.db.get_user_by_id(r.ClientID)
            client_name = client.FIO if client else "Неизвестно"
            request_list.append(f"{r.RequestID} - {r.CarModel} - {client_name}")
        
        selected_var = tk.StringVar()
        request_combo = ttk.Combobox(dialog, textvariable=selected_var, 
                                     values=request_list, width=50, state='readonly')
        request_combo.pack(pady=5)
        
        # Количество дней
        ttk.Label(dialog, text="Продлить на (дней):").pack(pady=5)
        ...
        def save():
            ...
            if not days_var.get().isdigit():
                messagebox.showwarning("Ошибка", "Введите число дней")
                return
            
            # Получаем ID заявки
            req_id = int(selected_var.get().split(' - ')[0])
            
            # Здесь в реальном проекте нужно обновить дату в БД
            # Пока просто показываем сообщение
            
            messagebox.showinfo("Успех", 
                f"Заявка №{req_id} продлена на {days_var.get()} дней\n"
                f"Клиент уведомлён о новом сроке")
            dialog.destroy()
        
        ttk.Button(dialog, text="✅ Продлить срок", command=save).pack(pady=15)
    
    def assign_additional_mechanic(self):
        """Привлечение дополнительного механика"""
        messagebox.showinfo("Информация", 
            "Уведомление отправлено всем свободным механикам.\n"
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("✅ МОДУЛЬ 2 (База данных) - реализован")
    print("✅ МОДУЛЬ 3 (QR-код + Менеджер качества) - реализован")
    print("="*60)
    print("\n🔑 НОВЫЙ ПОЛЬЗОВАТЕЛЬ ДЛЯ МОДУЛЯ 3:")
    print("   Логин: quality1")
    print("   Пароль: pass1")
    print("   Роль: Менеджер по качеству")
    print("\n" + "="*60)
    print("\n⚠️  Если QR-код не работает, установите библиотеки:")
